<%-- 
    Document   : SelectTipo
    Created on : 08/05/2015, 10:40:30
    Author     : Valdomiro
--%>
...
<% 
      ...
      if(session.getValue("loginUsuario") == null && session.getValue("senhaUsuario") == null){
       ...
       }else 
       {
           ...
           array2 = permis.split("-");
           if(array2[3].toString().charAt(5)=='0'){
             response.sendRedirect("Home.jsp");
           }
       }
 %>
...
<html>
    ...
    <% 
        ...
        try { 
              ...
              rs = st.executeQuery("select variedade from local_coleta where variedade <> ' '  group by variedade order by variedade;");
              
              %>
			   <input type="hidden" name="tradu" id="trad" value="<% out.print(res.traduz(285,traducao)); %>"/>
                 <div class="container">

	     <form id="contactform" name="f1" class="rounded" method="post" action="RelatorioVariedade.jsp" onSubmit="return selecao(); return false;">
                         ...
                         <option value="0"><% out.print(res.traduz(192,traducao)); %>..</option>
                            <%          
                                    while(rs.next())
                                    {
                                        ...
                                        %>
                                        <option value="<% out.print(rs.getString("variedade")); %>"  title="" name="tipo"><% out.print(rs.getString("variedade")); %></option>
                                        <%
