<%-- 
    Document   : SelectPais
    Created on : 28/05/2015, 13:53:00
    Author     : Valdomiro
--%>
...
<% 
      ...
      if(session.getValue("loginUsuario") == null && session.getValue("senhaUsuario") == null){
       ...
       }else 
       {
           ...
           array2 = permis.split("-");
           if(array2[2].toString().charAt(3)!='1'){
             response.sendRedirect("Home.jsp");
           }
       }
 %>
...
<html>
    ...
    <% 
        ...
        try { 
              ...
              rs = st.executeQuery("select pais from usuario where pais <>'' group by pais");
              
              %>
			  <input type="hidden" name="tradu" id="trad" value="<% out.print(res.traduz(285,traducao)); %>"/>
                 <div class="container">

	     <form id="contactform" name="f1" class="rounded" method="post" action="Rup.jsp" onSubmit="return selecao(); return false;">
                         ...
                         <option value="0"><% out.print(res.traduz(192,traducao)); %>..</option>
                            <%          
                                    while(rs.next())
                                    {
                                        ...
                                        %>
                                        <option value="<% out.print(rs.getString("pais")); %>"  title="" name="pais"><% out.print(rs.getString("pais")); %></option>
                                        <%
