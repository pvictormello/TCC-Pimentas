<%-- 
    Document   : SelectId
    Created on : 08/05/2015, 09:52:15
    Author     : Valdomiro
--%>
...
<% 
      ...
      if(session.getValue("loginUsuario") == null && session.getValue("senhaUsuario") == null){
       ...
       }else 
       {
           ...
           array2 = permis.split("-");
           if(array2[3].toString().charAt(3)=='0'){
             response.sendRedirect("Home.jsp");
           }
       }
 %>
   ...
   <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>
    ...
    <% 
        ...
        try { 
              ...
              rs = st.executeQuery("select IDAmostra from local_coleta order by IDAmostra");
              
              %>
			    <input type="hidden" name="tradu" id="trad" value="<% out.print(res.traduz(285,traducao)); %>"/>
                 <div class="container">

	     <form id="contactform" name="f1" class="rounded" method="post" action="RelatorioID.jsp" onSubmit="return selecao(); return false;">
                         ...
                         <option value="0"><% out.print(res.traduz(192,traducao)); %>..</option>
                            <%          
                                    while(rs.next())
                                    {
                                        ...
                                        %>
                                        <option value="<% out.print(rs.getString("IDAmostra")); %>"  title="" name="id"><% out.print(rs.getString("IDAmostra")); %></option>
                                        <%
