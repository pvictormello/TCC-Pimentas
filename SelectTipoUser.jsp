<%-- 
    Document   : SelectTipoUser
    Created on : 28/05/2015, 14:01:13
    Author     : Valdomiro
--%>
...
<% 
      ...
      if(session.getValue("loginUsuario") == null && session.getValue("senhaUsuario") == null){
       ...
       }else 
       {
           ...
           array2 = permis.split("-");
           if(array2[2].toString().charAt(4)!='1'){
             response.sendRedirect("Home.jsp");
           }
       }
 %>
...
<html>
    ...
    <% 
        ...
        try { 
              ...
              rs = st.executeQuery("select tipousuario from usuario where tipousuario <>'' group by tipousuario");
              
              %>
			    <input type="hidden" name="tradu" id="trad" value="<% out.print(res.traduz(285,traducao)); %>"/>
                 <div class="container">

	     <form id="contactform" name="f1" class="rounded" method="post" action="Rut.jsp" onSubmit="return selecao(); return false;">
                         ...
                         <option value="0"><% out.print(res.traduz(192,traducao)); %>..</option>
                            <%          
                                    while(rs.next())
                                    {
                                        ...
                                        %>
                                        <option value="<% out.print(rs.getString("tipousuario")); %>"  title="" name="tp"><% out.print(rs.getString("tipousuario")); %></option>
                                        <%
